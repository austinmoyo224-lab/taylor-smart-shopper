...
function _weatherTool() {
  return tool({
    description:
      "Look up verified CURRENT weather plus a 3-day forecast for ANY South African city, town or suburb. Always call this for a named location instead of using the subscriber's current-location weather. Also use it for destination weather and weather-aware meal ideas.",
    inputSchema: z.object({
      location: z
        .string()
        .min(2)
